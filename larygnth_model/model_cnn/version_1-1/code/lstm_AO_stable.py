...
class LSTM_AO_Architecture():
    ...
    def create_net(self):
        ...
        # shape helper
        shape_in = [self._xsize[0], self._xsize[1], self._ndimensions]
        shape_lstm_normal = [self._xsize[0], self._xsize[1], self._nfilters]
        shape_lstm_small = [self._xsize[0]//2, self._xsize[1]//2, self._nfilters]

        # block 0
        drop0_1 = tf.layers.dropout(in_node, rate=drop_rate_input)
                                
        # block 1
        if self._two_gpus:
            with tf.device('/device:GPU:1'):
                if self._gru:
                    rec1_1 = bcgru(drop0_1, "bcgru1_1", nfilters=self._nfilters, input_shape=shape_in, ksize=3)
                else:
                    rec1_1 = bclstm(drop0_1, "bclstm1_1", nfilters=self._nfilters, input_shape=shape_in, ksize=3)
        else:
            if self._gru:
                rec1_1 = bcgru(drop0_1, "bcgru1_1", nfilters=self._nfilters, input_shape=shape_in, ksize=3)
            else:
        ...
        if self._gru:
            rec1_2 = bcgru(rec1_1, "bcgru1_2", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
        else:
            rec1_2 = bclstm(rec1_1, "bclstm1_2", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
        
        pool1_1 = max_pool2d_from_3d(rec1_2)
        drop1_1 = tf.layers.dropout(pool1_1, rate=drop_rate_input)
        
        # block 2
        if self._two_gpus:
            with tf.device('/device:GPU:1'):
                if self._gru:
                    rec2_1 = bcgru(drop1_1, "bcgru2_1", nfilters=self._nfilters, input_shape=shape_lstm_small, ksize=3)
                else:
                    rec2_1 = bclstm(drop1_1, "bclstm2_1", nfilters=self._nfilters, input_shape=shape_lstm_small, ksize=3)
        else:
            if self._gru:
                rec2_1 = bcgru(drop1_1, "bcgru2_1", nfilters=self._nfilters, input_shape=shape_lstm_small, ksize=3)
            else:
        ...
        if self._gru:
            rec2_2 = bcgru(rec2_1, "bcgru2_2", nfilters=self._nfilters, input_shape=shape_lstm_small, ksize=3)
        else:
            rec2_2 = bclstm(rec2_1, "bclstm2_2", nfilters=self._nfilters, input_shape=shape_lstm_small, ksize=3)

        deconv2_1 = deconv2d_from_3d(rec2_2, nfilters=self._nfilters)
        drop2_1 = tf.layers.dropout(deconv2_1, rate=drop_rate_input)
        
        # block 3
        if self._two_gpus:
            with tf.device('/device:GPU:1'):
                if self._gru:
                    rec3_1 = bcgru(drop2_1, "bcgru3_1", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
                else:
                    rec3_1 = bclstm(drop2_1, "bclstm3_1", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
        else:
            if self._gru:
                rec3_1 = bcgru(drop2_1, "bcgru3_1", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
            else:
        ...
        if self._gru:
            rec3_2 = bcgru(rec3_1, "bcgru3_2", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)
        else:
            rec3_2 = bclstm(rec3_1, "bclstm3_2", nfilters=self._nfilters, input_shape=shape_lstm_normal, ksize=3)

        conv3_1 = conv2d_from_3d(rec3_2, self._nfilters, ksize=3, bn=self._bn, padding="same")
        
        final_conv = conv2d_from_3d(conv3_1, self._nclasses, ksize=1, padding="same", activation=None)
        output_node = final_conv
    ...
    def get_architecture_name(self):
        ...
        return "LSTM_AO_L{}_F{}_Dim{}_Seq{}_LF{}_LR{}_C{}_CW{}_DeR{}_BN{}_Reg{}_RegS{}_Std{}_CC{}_constF{}_GRU{}_MidS{}{}".format(self._nlayers, 
                                                       self._nfilters,
                                                       self._ndimensions,
                                                       self._nsequences,
                                                       self._loss_function, 
...
class LSTM_AO_Trainer():
    ...
    def train(self, lstm_net, unet_runner):
        ...
        with tf.Session() as sess:
            ...
            if not self._skip_val:
                self._validate(net, sess, 0, unet_runner, val_writer, val_merged_summary)
            ...
            for step in range(1, self._iterations_train+1):
                ...
                # ======================= run sequences through unet =========================
                new_sequences = []
                for i in range(len(batch_x)):
                    feature_maps = unet_runner.run_sticky_session(batch_x[i])
                    new_sequences.append(feature_maps)
                batch_x = np.stack(new_sequences, axis=0)   
                ...
                if step % self._iterations_per_epoch == 0:
                    # val
                    if not self._skip_val:
                        self._validate(net, sess, step, unet_runner, val_writer, val_merged_summary)
                    
            unet_runner.close()
            logging.info("Done.")
    
    def _validate(self, net, sess, cur_step, unet_runner, writer, summary):
        ...
        for step in range(self._iterations_valid):
            ...
            if self._save_model and step == self._iterations_valid-1:
                self._save_current_model(sess, original_batch_x, batch_y, pred)
                print()
    ...
    def _save_current_model(self, sess, x, y, p):
        ...
        #dummy_x = np.ones(shape=(x.shape[0], x.shape[1], x.shape[2], 3), dtype=int)
...
class LSTM_AO_Tester():
    ...
    def test(self, unet_runner, batch_size=1, save_validate_image=False):
    
        # runtime
        _runtime_load_batch = np.asarray([])
        _runtime_feedforward_batch_unet = np.asarray([])
        _runtime_feedforward_batch_lstm = np.asarray([])
        ...
        with tf.Session(graph=g) as sess:
            ...
            for i in range(self._dataprovider.dataset_length() // batch_size):
                ...
                for j in range(len(x)):
                    feature_maps = unet_runner.run_sticky_session(x[j])
                    new_sequences.append(feature_maps)
                x = np.stack(new_sequences, axis=0) 
                
                _runtime_t4_1 = time.time() # runtime
                ...
                if save_validate_image:
                    self._save_image(x_original, y, out_p, i)
        
                _runtime_t6 = time.time() # runtime
                
                # runtime
                _runtime_load_batch = np.append(_runtime_load_batch, _runtime_t4 - _runtime_t3)
                _runtime_feedforward_batch_unet = np.append(_runtime_feedforward_batch_unet, _runtime_t4_1 - _runtime_t4)
                _runtime_feedforward_batch_lstm = np.append(_runtime_feedforward_batch_lstm, _runtime_t5 - _runtime_t4_1)
        ...
        logging.info("Runtime: Feedforward Batch UNet: Sum {:.4f} Mean {:.4f} Std. {:.4f}".format(np.sum(_runtime_feedforward_batch_unet), np.mean(_runtime_feedforward_batch_unet), np.std(_runtime_feedforward_batch_unet)))
        logging.info("Runtime: Feedforward Batch LSTM: Sum {:.4f} Mean {:.4f} Std. {:.4f}".format(np.sum(_runtime_feedforward_batch_lstm), np.mean(_runtime_feedforward_batch_lstm), np.std(_runtime_feedforward_batch_lstm)))
        ...
        sio.savemat(path, {"lstm_ao_normal_load_batch":_runtime_load_batch,
                            "lstm_ao_normal_feedforward_batch_lstm":_runtime_feedforward_batch_lstm,
                            "lstm_ao_normal_feedforward_batch_unet":_runtime_feedforward_batch_unet,
                            "lstm_ao_normal_save_batch":_runtime_save_batch})
...
class LSTM_AO_Tester_MidSeq():
    ...
    def test(self, unet_runner, save_validate_image=False):
        ...
        with tf.Session(graph=g) as sess:
            ...
            for j in range(nsequences):
                ...
                for k in range(len(x)):
                    feature_maps = unet_runner.run_sticky_session(x[k])
                    new_sequences.append(feature_maps)
                x = np.stack(new_sequences, axis=0) 
                ...
                for i in range(50):
                    if i != 0:
                        ...
                        if i > 47:
                            ...
                            for k in range(len(_x)):
                                feature_maps = unet_runner.run_sticky_session(_x[k])
                                new_sequences.append(feature_maps)
                            _x = np.stack(new_sequences, axis=0) 
                    ...
                    if save_validate_image:
                        # TODO: x_original aktualisieren wenn 2 neue Bilder geladen werden;
                        # aktuell ist x_original immer nur die ersten 10 Bilder
                        output_x = x_original[:,4:6,...]
                        output_y = y[:,4:6,...]
                ...
                if save_validate_image:
                    logging.info("Achtung: Ausgabe des RGB Bildes nicht richtig! Bild nicht an allen Stellen aktuell.")
        ...
        path = self._output_path + "runtime.mat"
        sio.savemat(path, {"lstm_ao_midseq_load_batch":_runtime_load_batch,
                            "lstm_ao_midseq_feedforward_batch_lstm":_runtime_feedforward_batch_lstm,
                            "lstm_ao_midseq_feedforward_batch_unet":_runtime_feedforward_batch_unet,
                            "lstm_ao_midseq_save_batch":_runtime_save_batch})
